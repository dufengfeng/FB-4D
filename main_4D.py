...
from dataset_4d import SparseDataset,SparseDatasetWrapper,SparseDatasetWrapperALLVIEWS
...
class GUI:
    def __init__(self, opt, elevation_list, azimuth_list):
        self.elevation_list = elevation_list
        self.azimuth_list = azimuth_list
        ...
        if self.opt.view_path is not None:
            self.view_path = self.opt.view_path
        ...
        self.dataset = SparseDatasetWrapperALLVIEWS(self.opt, self.size, H=self.H, W=self.W, device=self.device)
    ...
    def prepare_image(self,idx):
        ...
        if self.input_imgs is not None:
            for i in np.arange(len(self.input_imgs)):
                #print(idx,i)
    ...
    def prepare_train(self):
        ...
        # default camera namely the camera matrix
    ...
    def train_step(self):
        ...
        for _ in range(self.train_steps):
            ...
            if self.step == 30001:
                self.save_renderings( 0, -75, 2 ,'eval_0')
                self.save_renderings( 0, 15, 2 ,'eval_1')
                self.save_renderings( 0, 105, 2 ,'eval_2')
                self.save_renderings( 0, 195, 2 ,'eval_3')
            ...
            for _ in np.arange(self.opt.batch_size):
                ...
                if hor<30 and hor>-30 or np.random.rand()>0.4:
                    ...
                    # choose the closest idx
                    min_distance = []
                    for now_index in range(len(self.azimuth_list)):
                        new_azi = abs(360-abs(hor-self.fixed_azimuth[now_index]))
                        if(new_azi>180):
                            new_azi = 360-new_azi
                        min_distance.append(new_azi)
                    min_value = min(min_distance)
                    min_indices = [i for i, value in enumerate(min_distance) if value == min_value]
                    best_j = min(min_indices, key=lambda j: abs(ver - self.fixed_elevation[j]))
                    idx = best_j
    ...
    def set_fix_camWrapper(self):
        self.fixed_cam_plus=[]
        self.fixed_elevation = []
        self.fixed_azimuth = []
        
        for i in range(len(self.azimuth_list)):
            self.fixed_elevation.append(-self.elevation_list[i])
            self.fixed_azimuth.append(self.azimuth_list[i])
            pose = orbit_camera(-self.elevation_list[i], self.azimuth_list[i], self.opt.radius)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--camera", required=True, help="path to the camera parameters")
    ...
    with open(args.camera, 'r') as file:
        camera_file = json.load(file)

    cam_nums = camera_file.get('photo-nums', 0)
    cam_params = camera_file.get('photos', [])
    
    elevation_list = []
    azimuth_list = []

    for i, cam_param in enumerate(cam_params):
        elevation = cam_param.get('elevation', 'N/A')
        azimuth = cam_param.get('azimuth', 'N/A')
        elevation_list.append(elevation)
        azimuth_list.append(azimuth)

    gui = GUI(opt, elevation_list=elevation_list, azimuth_list=azimuth_list)
    if opt.gui:
        gui.render()
    else:
