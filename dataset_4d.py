...
class SparseDatasetWrapper:
    ...
    def collate_zero123(self,index):
        ...
        self.pattern=os.path.join(self.path,'zero123/{}_rgba/*.png'.format(str(index)))
        self.pattern2=os.path.join(self.path,'zero123new/{}_rgba/*.png'.format(str(index)))
        self.input_imgs=[]
        self.input_masks=[]
        file_list = glob.glob(self.pattern)
        file_list2 = sorted(glob.glob(self.pattern2))

        #print(self.pattern,file_list)
        for index, files in enumerate(sorted(file_list)):
                    #print(f'[INFO] load image from {files}...')
                    img = cv2.imread(files, cv2.IMREAD_UNCHANGED)
                    if img.shape[-1] == 3:
                        if self.bg_remover is None:
                    ...
                    # load second image for different zero123 version
                    img_2 = cv2.imread(file_list2[index], cv2.IMREAD_UNCHANGED)
                    if img_2.shape[-1] == 3:
                        if self.bg_remover is None:
                            self.bg_remover = rembg.new_session()
                        img_2 = rembg.remove(img_2, session=self.bg_remover)

                    img_2 = cv2.resize(img_2, (self.W, self.H), interpolation=cv2.INTER_AREA)
                    img_2 = img_2.astype(np.float32) / 255.0

                    self.input_mask_2 = img_2[..., 3:]
                    # white bg
                    self.input_img_2 = img_2[..., :3] * self.input_mask_2 + (1 - self.input_mask_2)
                    # bgr to rgb
                    self.input_img_2 = self.input_img_2[..., ::-1].copy()

                    self.input_imgs.append(self.input_img_2)
                    self.input_masks.append(self.input_mask_2)
...
class SparseDatasetWrapperALLVIEWS:
    def __init__(self, opt, size,device='cuda', type='train', H=256, W=256):
        super().__init__()
    ...
    def collate_ref(self,index):
        #print(index,str(index))
        file = os.path.join(self.path,'ref/{:02d}_rgba.png'.format(int(index)))
        #print(f'[INFO] load image from {file}...')
    ...
    def collate_zero123(self,index):
        ...
        self.pattern=os.path.join(self.path,'zero123/{:02d}_rgba/*.png'.format(int(index)))
        self.input_imgs=[]
        self.input_masks=[]
        file_list = glob.glob(self.pattern)
...
class SparseDatasetWrapperALLVIEWS2:
    def __init__(self, opt, size,device='cuda', type='train', H=256, W=256):
        super().__init__()
    ...
    def collate_ref(self,index):
        #print(index,str(index))
        file = os.path.join(self.path,'ref/{}_rgba.png'.format(int(index)))
        #print(f'[INFO] load image from {file}...')
    ...
    def collate_zero123(self,index):
        ...
        self.pattern=os.path.join(self.path,'zero123/{}_rgba/*.png'.format(int(index)))
        self.input_imgs=[]
        self.input_masks=[]
        file_list = glob.glob(self.pattern)
